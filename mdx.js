...
function decodeEntity(characters) {
  return own5.call(characterEntities, characters) ? characterEntities[characters] : false;
}
...
function parseEntities(value, options = {}) {
  ...
  while (++index2 <= value.length) {
    if (character === 10) {
      column = indent2 && indent2[lines] || 1;
    }
    ...
    if (character === 38) {
      ...
      entity = "";
      characters = "";
      test = type === "named" ? isAlphanumerical : type === "decimal" ? isDecimal : isHexadecimal;
      end--;
      while (++end <= value.length) {
        ...
        if (type === "named" && own6.call(characterEntitiesLegacy, characters)) {
      ...
      if (!terminated && options.nonTerminated === false) {
        ...
        referenceCode = Number.parseInt(characters, type === "hexadecimal" ? 16 : 10);
  ...
  function now() {
    return {
      line,
      column,
      offset: index2 + (pos && pos.offset || 0)
    };
  }
...
function extension(combined, extension2) {
  ...
  for (key in extension2) {
    if (own7.call(extension2, key)) {
      ...
      const maybe = own7.call(combined, key) ? combined[key] : void 0;
...
function unknown(h, node) {
  ...
  if ("value" in node && !(own8.call(data3, "hName") || own8.call(data3, "hProperties") || own8.call(data3, "hChildren"))) {
...
function definitions(node) {
  ...
  function ondefinition(definition2) {
    ...
    if (id && !own9.call(cache, id)) {
      cache[id] = definition2;
    }
  }
  function getDefinition(identifier) {
    const id = clean(identifier);
    return id && own9.call(cache, id) ? cache[id] : null;
  }
}
