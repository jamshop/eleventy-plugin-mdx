        ...
        return {
          contents: (
            await mdx.compile(content, {
              jsxRuntime: "classic",
              rehypePlugins: globalStore.rehypePlugins,
              remarkPlugins: globalStore.remarkPlugins,
            })
          ).value,
          loader: "jsx",
        };
...
class EleventyMDX {
  constructor({ includeCDNLinks, components, rehypePlugins, remarkPlugins }) {
    globalStore.includeCDNLinks = includeCDNLinks;
    globalStore.components = components;
    globalStore.rehypePlugins = rehypePlugins;
    globalStore.remarkPlugins = remarkPlugins;
  }
  ...
  compile(permalink, inputPath) {
    return async function (props) {
      ...
      const ROOT_ID = `MDX_ROOT_${
        process.env.NODE_ENV !== "test" ? uuid() : "test"
      }`;
      ...
      const rootComponent = React.createElement(
        "div",
        { id: ROOT_ID },
        React.createElement(___mdx_component, props)
      );
      ...
      if (!serializeEleventyProps) {
        ...
        if (htmlTemplate) {
          if (typeof htmlTemplate === "function")
            return htmlTemplate(content, props);
